...
const LandingPage = () => {
  ...
  // Efecto para cargar usuario guardado
  useEffect(() => {
  ...
  if (savedUser) {
    try {
      ...
      setCurrentUser(user);
      setIsLoggedIn(true);
      // No establecemos showUserPanel en true inicialmente
  ...
  // Efecto para cargar servicios del usuario
  useEffect(() => {
  ...
  const handleUpdateUser = async () => {
    ...
    if (!phoneRegex.test(editedUserData.telefono)) {
      showNotification('error', 'El formato del teléfono no es válido. Debe ser un número español válido.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification('error', data.error || 'Error al actualizar los datos');
      }
    } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      loadUserServices(userData.username);
      setLoginData({ username: '', password: '' });
      showNotification('success', '¡Inicio de sesión exitoso!');
      setShowLoginModal(false);
    } else {
      showNotification('error', data.error || 'Error al iniciar sesión');
    }
  } catch (error) {
    console.error('Error:', error);
    showNotification('error', 'Error al conectar con el servidor');
  ...
  // Manejar contratación de servicio
  const handleContratarServicio = async (serviceName) => {
  console.log('Intentando contratar el servicio:', serviceName); // Debug

  if (!isLoggedIn) {
    showNotification('info', 'Debes iniciar sesión para contratar servicios');
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.error || 'Error al contratar el servicio');
    }

    // Si llegamos aquí, la contratación fue exitosa
    showNotification('success', 'Servicio contratado correctamente');
    await loadUserServices(currentUser.username);

  } catch (error) {
    console.error('Error:', error);
    showNotification('error', error.message || 'Error al contratar el servicio');
  }
};
  ...
  const handleRegister = async (e) => {
    ...
    if (!passwordRegex.test(registerData.password)) {
      showNotification('error', 'La contraseña no cumple con los requisitos');
      return;
    }
    ...
    if (!phoneRegex.test(registerData.telefono)) {
      showNotification('error', 'El teléfono debe tener exactamente 9 dígitos');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification('error', data.error || 'Error al registrar usuario');
      }
    } catch (error) {
      console.error('Error:', error);
      showNotification('error', 'Error al conectar con el servidor');
...
>
  ...
  }, [showRegister, isPaused, heroSections.length]);
  return (
    ...
    <div className="flex items-center">
	  ...
	  {/* Background Image */}
        ...
        {isLoggedIn && showUserPanel ? (
          ...
          // Registration form - Fixed syntax
