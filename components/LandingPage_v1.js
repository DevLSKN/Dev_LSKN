...
const LandingPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setCurrentUser(data.user);  // Guardamos la información del usuario
        setIsLoggedIn(true);       // Actualizamos el estado de login
        setLoginData({            // Limpiamos el formulario
          username: '',
          password: ''
        });
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="bg-white shadow-md p-4">
        <div className="w-full flex justify-between items-center px-4">
          ...
          <div className="flex items-center">
            {isLoggedIn ? (
              // Botón de usuario cuando está logueado
      ...
      <main className="flex-grow relative">
        ...
        {isLoggedIn ? (
          // Panel de Usuario integrado en la Hero Unit
          <div className="absolute inset-0 flex items-center">
            <div className="text-white p-6 ml-20 w-full max-w-4xl">
