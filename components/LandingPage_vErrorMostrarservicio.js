...
const handleUseService = async (serviceId) => {
  console.log('ID del servicio a usar:', serviceId); // Debug
  ...
  try {
    ...
    if (response.ok) {
      ...
      showToast(data.error || 'Error al utilizar el servicio', 'error');
    }
  } catch (error) {
...
const handleLogin = async (e) => {
  ...
  console.log('Servicio a renderizar:', service); // Debug - ver el servicio completo
  return (
    <div 
      key={service._id}
    ...
    >
      <div>
        ...
        <p className="text-sm">ID: {service._id}</p> {/* Debug - mostrar el ID */}
        <p className="text-sm">
      ...
      {['DAY PASS', 'BONO (5 DAY PASS)', 'BONO (10 DAY PASS)'].includes(service.servicio) && (
        <button
          onClick={() => {
            console.log('Click en servicio:', service); // Debug
            handleUseService(service._id);
          }}
          className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600"
        >
          Usar
