...
// Reemplaza las funciones 	in y handleContratarServicio con estas versiones:
...
const handleContratarServicio = async (serviceName) => {
  ...
  try {
    ...
    await apiServices.contractService(currentUser.username, serviceName);
...
const handleUseService = async (serviceId) => {
  ...
  if (usosActuales >= maxUsos) {
    showToast('No quedan usos disponibles para este servicio', 'error');
    return;
  }

  const confirmar = window.confirm('¿Estás seguro de que deseas usar este servicio hoy?');
  if (!confirmar) return;
  ...
  try {
    ...
    showToast('Servicio utilizado correctamente', 'success');
...
const ServiceCard = ({ service, onUseService }) => {
  // Función para obtener el máximo de usos según el tipo de servicio
  const getUsosMaximos = (serviceName) => {
    if (serviceName.includes('BONO (10 DAY PASS)')) return 10;
    if (serviceName.includes('BONO (5 DAY PASS)')) return 5;
    if (serviceName.includes('DAY PASS')) return 1;
    return null; // Para otros tipos de servicios
  };

  // Calcular usos actuales y máximos
  ...
  return (
    <div className="bg-white bg-opacity-10 p-4 rounded-lg hover:bg-opacity-20 transition-all">
      <div className="flex justify-between items-start mb-2">
        ...
        {usosMaximos !== null && (
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            usosRestantes > 0 ? 'bg-blue-500 text-white' : 'bg-red-500 text-white'
          }`}>
      ...
      <div className="text-sm space-y-2">
        <p>
          <span className="text-gray-300">Fecha de compra:</span>{' '}
          {formatearFecha(service.createdAt)}
        </p>
        
        {usosActuales > 0 && (
          <p>
            <span className="text-gray-300">Último uso:</span>{' '}
            {formatearFecha(service.usos[service.usos.length - 1].fecha)}
          </p>
        )}

        {usosMaximos !== null && (
          <p className={`font-medium ${usosRestantes > 0 ? 'text-green-400' : 'text-red-400'}`}>
            {usosRestantes} {usosRestantes === 1 ? 'uso' : 'usos'} restante{usosRestantes !== 1 ? 's' : ''}
          </p>
        )}
      </div>
      ...
      if (pendingService) {
        ...
        try {
          ...
          if (serviceResponse.ok) {
            ...
            showToast(errorData.error || 'Error al contratar el servicio', 'error');
          }
        } catch (error) {
          console.error('Error:', error);
          showToast('Error al contratar el servicio: ' + error.message, 'error');
        }
      } else {
        setShowLoginModal(false);
        setShowUserPanel(true);
...
// Componente mejorado para mostrar el historial de servicios
const ServiceHistory = ({ services, onUseService }) => {
  if (!Array.isArray(services)) {
    console.error('Services must be an array:', services);
    return <p className="text-center text-gray-300">Error al cargar servicios</p>;
  }

  if (services.length === 0) {
    return <p className="text-center text-gray-300">No hay servicios contratados</p>;
  }
      ...
      {services.map((service) => (
        ...
        >
          <div>
            ...
            <p className="text-sm">
              Contratado: {new Date(service.createdAt).toLocaleDateString('es-ES')}
            </p>
            {service.usos && (
              <p className="text-sm">
                Usos: {service.usos.length} / 
                {service.servicio.includes('5') ? '5' : 
                 service.servicio.includes('10') ? '10' : '1'}
              </p>
            )}
          </div>
          {['DAY PASS', 'BONO (5 DAY PASS)', 'BONO (10 DAY PASS)'].includes(service.servicio) && (
            <button
              onClick={() => onUseService(service._id)}
              className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600"
              disabled={service.usos?.length >= (service.servicio.includes('5') ? 5 : 
                        service.servicio.includes('10') ? 10 : 1)}
            >
              Usar
            </button>
