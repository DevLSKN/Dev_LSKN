...
const LandingPage = () => {
  ...
  const handleContratarServicio = async (serviceName) => {
    console.log(`Intentando contratar el servicio: ${serviceName}`); // Verificar si el evento onClick se está registrando
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(data.error || 'Error al contratar el servicio');
      }
    } catch (error) {
      console.error('Error:', error); // Verificar si hay algún error en la conexión con la API
      alert('Error al conectar con el servidor');
    }
  };
        ...
        <div className="grid grid-cols-3 gap-6 w-full max-w-6xl">
          {[
            ...
            <div key={index} className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all flex flex-col">
              ...
              <button
                onClick={() => {
                  console.log(`Botón de contratar clicado para: ${servicio.name}`); // Verificar si el botón fue clicado
                  handleContratarServicio(servicio.name);
                }}
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-lg shadow-xl w-96">
          ...
          <form onSubmit={async (e) => {
            e.preventDefault();
            const success = await handleLogin(loginModalData);
            if (success) {
