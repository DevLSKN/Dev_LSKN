...
const LandingPage = () => {
  ...
  const handleUpdateUser = async () => {
    ...
    if (!phoneRegex.test(editedUserData.telefono)) {
      alert('El formato del teléfono no es válido. Debe ser un número español válido.');
      return;
    }
  ...
  content: (
    <div className="grid grid-cols-3 gap-6 w-full max-w-6xl">
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">DAY PASS</h3>
        <p className="text-lg">Acceso diario completo a todas nuestras instalaciones</p>
      </div>
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">BONO (5 DAY PASS)</h3>
        <p className="text-lg">Pack de 5 accesos para usar cuando quieras</p>
      </div>
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">BONO (10 DAY PASS)</h3>
        <p className="text-lg">Pack de 10 accesos con precio especial</p>
      </div>
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">MENSUALIDAD</h3>
        <p className="text-lg">Acceso ilimitado durante un mes</p>
      </div>
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">ENTRENAMIENTO PERSONAL</h3>
        <p className="text-lg">Sesiones personalizadas con nuestros profesionales</p>
      </div>
      <div className="bg-black bg-opacity-50 p-6 rounded-lg hover:bg-opacity-60 transition-all">
        <h3 className="text-2xl font-bold mb-2">DIETA + RUTINA</h3>
        <p className="text-lg">Plan personalizado de nutrición y entrenamiento</p>
      </div>
    </div>
  )
  },
                  ...
                  {userServices.map((service, index) => (
                    ...
                    >
                      ...
                      <span className={`px-2 py-1 rounded text-sm ${
                        service.status === 'activo' ? 'bg-green-500' :
                        service.status === 'usado' ? 'bg-gray-500' : 'bg-red-500'
                      }`}>
                        {service.status}
                      </span>
                    </div>
