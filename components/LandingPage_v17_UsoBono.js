...
const ServiceCard = ({ service, onUseService }) => {
  console.log('ServiceCard recibiendo:', {
      ...
      <div className="flex justify-between items-start mb-2">
        <div>
          ...
          <p className="text-sm text-gray-300">
            Comprado: {new Date(service.createdAt).toLocaleDateString('es-ES')}
          </p>
        </div>
        {usosMaximos !== null && (
      ...
      {service.usos && service.usos.length > 0 && (
        <div className="mt-2 text-sm">
          <p className="text-gray-300">
            Último uso: {new Date(service.usos[service.usos.length - 1].fecha).toLocaleDateString('es-ES')}
          </p>
        </div>
      )}
